...
{
    public partial class FormViewVisitorMoreDetailsToSecurity : Form
    {
        SqlConnection con = new SqlConnection(@"Data Source=NISH\SQLSERVER;Initial Catalog=VMS;Integrated Security=True");

        public FormViewVisitorMoreDetailsToSecurity(string visitorId)
        {
            InitializeComponent();
            textBoxVisitorId.Text = visitorId;
        }

        private void FormViewVisitorMoreDetailsToSecurity_Load(object sender, EventArgs e)
        {
            try
            {
                con.Open();
        ...
        private void buttonViewAppointment_Click(object sender, EventArgs e)
        {
            try
            {
                FormViewVisitorAppointmentsToSecurity appForm = new FormViewVisitorAppointmentsToSecurity();
