...
{
    public partial class FormViewVisitorMoreDetails : Form
    {
        SqlConnection con = new SqlConnection(@"Data Source=NISH\SQLSERVER;Initial Catalog=VMS;Integrated Security=True");

        public FormViewVisitorMoreDetails(string visitorId)
        {
            InitializeComponent();
            textBoxVisitorId.Text = visitorId;
        }

        private void FormViewVisitorMoreDetails_Load(object sender, EventArgs e)
        {
            try
            {
                con.Open();
        ...
        {
            AppointmentsForm newfrm = new AppointmentsForm();
