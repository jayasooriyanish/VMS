...
{
    public partial class FormViewHistoryFromViewVisitorToEmployee : Form
    {
        SqlConnection con = new SqlConnection(@"Data Source=NISH\SQLSERVER;Initial Catalog=VMS;Integrated Security=True");

        public FormViewHistoryFromViewVisitorToEmployee(string vid)
        {
            InitializeComponent();
            textBoxVisitorId.Text = vid;
        }

        private void FormViewHistoryFromViewVisitorToEmployee_Load(object sender, EventArgs e)
        {
            try
            {
                con.Open();
